...
(define-with-return
  ...
  (work_drawables nil)
    ...
    ; since v3, get a count and vector of drawables
    (set! work_drawables (cadr (gimp-image-get-selected-drawables tempImage)))
    ; but we only heal one
    ; TODO yell at user if many drawables are chosen
    (set! work_drawable (vector-ref work_drawables 0))
    ...
    (set! newWidth  (min newWidth  (- (car (gimp-image-get-width  tempImage)) newLLX)))
    (set! newHeight (min newHeight (- (car (gimp-image-get-height tempImage)) newLLY)))
